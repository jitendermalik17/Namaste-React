    ...
    return (
        <div className="header-container bg-orange-100 shadow-lg">
           ...
           <span className="font-bold text-green-700">{user.name}</span>
            ...
            {navObj.map(curNav => <li key={curNav.id}><Link className={active === curNav.id ? "active" : "a"} to={curNav.url} onClick={()=> setActive(curNav.id)}>{curNav.navName} {curNav.id === 4 && `- ${cartItems.length} Items`} </Link></li>)}
